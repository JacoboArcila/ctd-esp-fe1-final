import { getAllCharacters, searchCharactersAPI } from "../../services/character.service";
import {
  ChangePageAction,
  CharactersActions,
...
} from "../../types/characters.actions";
import { ActionCreator, ThunkAction } from "@reduxjs/toolkit";
import { Character } from "../../types/characters.type";
import { GlobalState } from "../store";

const initialFetchCharacters: ActionCreator<InitialFetchOfCharactersAction> = (
  page: number
) => {
  return {
...
const initialFetchCharactersSuccess: ActionCreator<
  InitialFetchOfCharactersSuccessAction
...
const initialFetchCharactersError: ActionCreator<
  InitialFetchOfCharactersErrorAction
> = (error: string) => {
  return {
...
export const toggleFavorites: ActionCreator<ToggleFavoriteAction> = (
  character: Character
) => {
  return {
...
export const removeAllCharactersFromFavorites: ActionCreator<
  RemoveAllFromFavoritesAction
> = () => {
  return {
...
interface SearchCharactersThunkAction
  extends ThunkAction<void, GlobalState, unknown, CharactersActions> {}
